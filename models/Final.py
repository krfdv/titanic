...
def main():
    ...
    X_train = scaler.fit_transform(X_train)
    X_test = scaler.transform(X_test)
    y_test = best_models["Random Forest"].predict(X_test)
    df["Survived"] = y_test
    df.to_csv("data/processed/submission.csv", index=False)
    df = pd.read_csv("../data/processed/submission.csv")
    #remove from submission.csv this column Pclass,Age,SibSp,Parch,Fare,Sex_female,Embarked_C,Embarked_S
    df = df.drop(["Pclass", "Age", "SibSp", "Parch", "Fare", "Sex_female", "Embarked_C", "Embarked_S"], axis=1)
    #add from test.csv column PassengerId
    dt = pd.read_csv("../data/raw/test.csv")
    df["PassengerId"] = dt["PassengerId"] 
    df.to_csv("../data/processed/first_submission.csv", index=False)
